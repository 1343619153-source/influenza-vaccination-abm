...
def _population_weights_for_ili_lambda(
    ...
    if pc < 0 or pa < 0 or pe < 0:
        raise ValueError("Population proportions cannot be negative")
    s = pc + pa + pe
    if abs(s - 1.0) > 1e-6:
        raise ValueError(f"π_c+π_a+π_e must equal 1; current value: {s}")
    ...
    if rc <= 0 or ra <= 0 or re <= 0:
        raise ValueError("Published attack rate r must be positive")

    wc, wa, we = rc * pc, ra * pa, re * pe
    w_sum = wc + wa + we
    if w_sum <= 0:
        raise ValueError("W = Σ r_a·π_a must be positive")
...
def add_school_connections(nodes, child_indices, rng=None):
    ...
    if total_ratio <= 0:
        raise ValueError("???? student_ratio ??????")
    if abs(total_ratio - 1.0) > 0.01:
        print(f"?????????????? {total_ratio:.3f}????1.0??????")
    ...
    for cat in categories:
        ...
        if school_count <= 0:
            raise ValueError(f"???? {category_name} ? count ????")
...
def print_prior_vaccination_summary(nodes) -> None:
    ...
    cfg = PRIOR_VACCINATION_CONFIG or {}
    if not cfg.get('enabled', True):
        print("\nPrior vaccination history is disabled (PRIOR_VACCINATION_CONFIG['enabled']=False)")
        return
...
def print_initial_health_summary(summary):
    total = summary['total']
    print("\n=== Initial health status distribution ===")
...
def load_weekly_ili_caches():
    ...
    if df.shape[0] == 0 or df.shape[1] < 2:
        _ili_csv_fail("The file is empty or has insufficient columns")
        return
    ...
    if df.shape[1] >= 9:
        ...
        if not rows_ordered:
            _ili_csv_fail("The wide-format table has no valid data rows")
            return
    ...
    if not weeks:
        _ili_csv_fail("The narrow-format table has no valid weekly data rows")
        return
...
def print_prior_vaccination_ratio_diagnostic(
    ...
    cfg = PRIOR_VACCINATION_CONFIG or {}
    if not cfg.get('enabled', True):
        print("\nPrior-history vaccination multiplier is disabled (PRIOR_VACCINATION_CONFIG['enabled']=False)")
        return
    if formula_params is None:
        formula_params = FORMULA_PARAMS

    print("\n=== Prior vaccination history: vaccination-rate multiplier (median features, P_prior / P_no_prior)===")
    print("  Condition: H=0 (ILI, peer, policy, and cognition at typical levels); P_no_prior=σ(b), P_prior=σ(b+κ)")
    print(f"  {'Group':<6} {'P_no_prior':>10} {'P_prior':>10} {'Ratio':>10} {'Published OR':>10} {'prior κ':>12}")
    print("  " + "-" * 64)

    for grp, label in (('child', 'child'), ('elderly', 'elderly')):
...
def simulate_vaccination(nodes, rng=None, month_params=None, formula_params=None, step: int = 1):
    ...
    if formula_params is None:
        raise ValueError("The new vaccination rule is enabled, but fitted parameters are not loaded.")
...
def run_epidemic_simulation(nodes, num_steps=100, display_interval=5, verbose=False, rng=None, sample_steps=None,
    ...
    formula_params = VACCINATION_FORMULA_PARAMS_CACHE
    if formula_params is None:
        raise ValueError("Failed to enable the new vaccination rule: vaccination parameters are not loaded.")
    ...
    for step in range(1, num_steps + 1):
        ...
        if not is_daytime:
            vac_new = simulate_vaccination(
                nodes, rng=rng, month_params=month_params, formula_params=formula_params, step=step
            )
        else:
...
def print_monthly_increments(monthly_increments):
    ...
    print(
        "Note: vaccination columns sum calendar-day additions, using the same rule as the daily new-vaccination curve, over the days in month m ("
        f"{DAYS_PER_MONTH} days), then average across runs; this equals summing the plotted daily means within each month. "
        "Legacy data without per-step *_vaccinated_new fields still use the month-end vaccination stock difference."
    )
    if has_vac_ci:
        print("Vaccination columns use: mean [95% CI lower, upper], based on the t distribution across runs and consistent with the plot.")
    print("=" * 100)

    col_w = 28 if has_vac_ci else 15
    print(
        f"\n{'Month':<10} {'New child vaccinations':<{col_w}} {'New adult vaccinations':<{col_w}} "
        f"{'New elderly vaccinations':<{col_w}} {'Total new infections':<15}"
    )
    print("-" * (10 + col_w * 3 + 15))
    ...
    for month_idx in sorted(monthly_increments.keys()):
        ...
        print(f"  New elderly vaccinations: {_fmt_mean_ci(data, 'elderly_vaccinated_new')} people")
...
def print_group_prediction_metrics(
    monthly_increments: Dict[int, Dict[str, Any]],
    ...
    group_specs = [
        ('child', 'child', 'child_vaccinated_new'),
        ('adult', 'adult', 'adult_vaccinated_new'),
        ('elderly', 'elderly', 'elderly_vaccinated_new'),
    ]

    print("\n" + "=" * 80)
    title = "Group-specific point-prediction errors and mean confidence-interval width (full forecast period)"
    if scenario_label:
        title = f"[{scenario_label}]{title}"
    print(title)
    print(
        "MAE_g=(1/M)Σ|ŷ−y|, RMSE_g=√((1/M)Σ(ŷ−y)²), "
        f"sMAPE_g=(100%/M)Σ[2|ŷ−y|/(|ŷ|+|y|+ε)], ε={SMAPE_EPSILON:g}"
    )
    print("Mean CI width W̄_g=(1/M)Σ(U−L)")
    print("=" * 80)

    results: Dict[str, Dict[str, float]] = {}
    for key, label, field in group_specs:
        pred = [float(monthly_increments[m].get(field, np.nan)) for m in month_indices]
        real = [float(x) for x in list(real_values.get(key, []))[: len(pred)]]
        n = min(len(pred), len(real))
        if n == 0:
            print(f"  {label}: no valid months; skipped")
            continue

        y_hat = np.asarray(pred[:n], dtype=float)
        y = np.asarray(real[:n], dtype=float)
        mae = float(np.mean(np.abs(y_hat - y)))
        rmse = float(np.sqrt(np.mean((y_hat - y) ** 2)))
        smape = float(
            100.0
            * np.mean(
                2.0 * np.abs(y_hat - y) / (np.abs(y_hat) + np.abs(y) + SMAPE_EPSILON)
            )
        )

        lo_key, hi_key = f"{field}_ci_lower", f"{field}_ci_upper"
        widths = []
        for m in month_indices[:n]:
            row = monthly_increments[m]
            if lo_key in row and hi_key in row:
                widths.append(max(0.0, float(row[hi_key]) - float(row[lo_key])))
        ...
        if widths:
            print(
                f"  {label}:  MAE={mae:.4f}, RMSE={rmse:.4f}, "
                f"sMAPE={smape:.2f}%, Mean CI width={mean_w:.4f}"
            )
        else:
            print(
                f"  {label}:  MAE={mae:.4f}, RMSE={rmse:.4f}, "
                f"sMAPE={smape:.2f}%, Mean CI width=n/a"
            )
...
def plot_monthly_vaccination_ci_by_group(
    ...
    group_specs = [
        (
            ...
            'orange',
            'child_monthly_new_vaccination_ci.png',
        ),
        (
            'adult',
            'adult',
            ...
            'blue',
            'adult_monthly_new_vaccination_ci.png',
        ),
        (
            'elderly',
            'elderly',
            ...
            'red',
            'elderly_monthly_new_vaccination_ci.png',
        ),
    ]
    ...
    ci_note = (
        f"{num_runs}-run 95% confidence interval vs observed value"
    ...
    for group_key, group_name, prefix, color, filename in group_specs:
        ...
        if len(rv_list) < len(month_indices):
            print(
                f"Warning: {group_name} has only {len(rv_list)} observed values; "
                f"required: {len(month_indices)} observed values; missing months will not show observed points."
            )
        ...
        for i, m_idx in enumerate(month_indices):
            ...
            status = "inside interval" if inside else "outside interval"
            print(
        ...
        if in_x:
            plt.scatter(
                ...
                edgecolors='black',
                linewidths=0.8,
                label='Observed value (inside interval)',
            )
        if out_x:
            plt.scatter(
                out_x,
                ...
                edgecolors='black',
                linewidths=0.8,
                label='Observed value (outside interval)',
            )
        ...
        plt.xlabel('Month (Aug-Mar)', fontsize=12)
        ...
        print(f"{group_name} monthly new-vaccination confidence-interval plot saved to: {abs_path}")
        plt.show()
        plt.close()
...
def export_monthly_vaccination_ci_csv(
    ...
    try:
        ...
        print(f"Warning: unable to create CSV output directory '{output_dir}': {e}")
        return None
...
def export_to_excel(time_series_data, filename=None, sheet_name=None):
    ...
    data = {
        'Step': [data['step'] for data in time_series_data],
        'Infected': [data['infected'] for data in time_series_data],
        'Vaccinated': [data['vaccinated'] for data in time_series_data],
        'Recovered': [data['recovered'] for data in time_series_data]
    }
    ...
    try:
        ...
        if file_exists:
            ...
            abs_file_path = os.path.abspath(file_path)
            print(f"Simulation results exported to worksheet in new file '{sheet_name}': {abs_file_path}")
        ...
        print(f"Simulation results exported to: {abs_file_path} (CSV format; can be opened in Excel)")
        print("Tip: to export directly to Excel with multiple worksheets, run: pip install openpyxl")
...
def print_family_distribution(family_dist):
    ...
    print(f"  Type B (elderly living alone, 1 person): {family_dist['B']} households")

    print(f"\nNuclear families (without children):")
    print(f"  Type C (adult couple, 2 people): {family_dist['C']} households")
    print(f"  Type D (elderly couple, 2 people): {family_dist['D']} households")
    ...
    print(f"  Type G (parents + 1 child + 1 elderly person, 4 people): {family_dist['G']} households")
    print(f"  Type H (parents + 2 children + 1 elderly person, 5 people): {family_dist['H']} households")

    print(f"\nTotals:")
    print(f"  Total households: {family_dist['total_families']} households")
    ...
    if abs(family_dist['total_population'] - family_dist['expected_population']) > 1:
        print(f"  WARNING: population mismatch; difference: {family_dist['total_population'] - family_dist['expected_population']} people")
    print(f"  Average household size: {family_dist['avg_family_size']:.2f} people/household")

    print(f"\nResource utilization:")
    print(f"  Children used: {family_dist['children_used']} people (expected: {family_dist['expected_children']} people)")
    if abs(family_dist['children_used'] - family_dist['expected_children']) > 0:
        print(f"  ⚠️ child-count difference: {family_dist['children_used'] - family_dist['expected_children']} people")
    ...
    print(
        f"  Total adults used: {family_dist['parents_used'] + family_dist['singles_used']} people (expected: {family_dist['expected_adults']} people)")
    if abs((family_dist['parents_used'] + family_dist['singles_used']) - family_dist['expected_adults']) > 0:
        print(
            f"  ⚠️ adult-count difference: {(family_dist['parents_used'] + family_dist['singles_used']) - family_dist['expected_adults']} people")

    print(f"  Elderly people used: {family_dist['elderly_used']} people (expected: {family_dist['expected_elderly']} people)")
    if abs(family_dist['elderly_used'] - family_dist['expected_elderly']) > 0:
        print(f"  ⚠️ elderly-count difference: {family_dist['elderly_used'] - family_dist['expected_elderly']} people")
...
PRIOR_VACCINATION_CONFIG = {
    'enabled': True,
    'coverage_child': 0.1749,
    'coverage_adult': 0.0105,
    'coverage_elderly': 0.0066,
}
...
ILI_CONFIG = {
    'weekly_csv': os.path.join(PROJECT_DIR, 'data', 'weekly_ili_historical_inputs.csv'),
...
OUTPUT_CONFIG = {
    ...
    'enable_policy_half_month_scenarios': False,
}



POLICY_SHIFT_SCENARIO_BASE = (0.0, 'baseline')
POLICY_SHIFT_SCENARIO_ADVANCE = (-0.5, 'policy_advance_half_month')
POLICY_SHIFT_SCENARIO_DELAY = (0.5, 'policy_delay_half_month')


def build_policy_shift_scenarios(enable_half_month: Optional[bool] = None) -> List[Tuple[float, str]]:

    if enable_half_month is None:
        enable_half_month = bool(OUTPUT_CONFIG.get('enable_policy_half_month_scenarios', True))
    scenarios: List[Tuple[float, str]] = [POLICY_SHIFT_SCENARIO_BASE]
    if enable_half_month:
        scenarios.extend([POLICY_SHIFT_SCENARIO_ADVANCE, POLICY_SHIFT_SCENARIO_DELAY])
    return scenarios
...
def print_survey_weight_summary(
    ...
    print("Formula: z = b_g + β·ili_trend + η·ili_relative + γ·B_neighbor + policy_m + prior_g·I")
    print("=" * 72)

    print(
        f"Common coefficients: β={float(formula_params.get('beta_ili_trend', 0.0)):.3f}, "
        f"η={float(formula_params.get('eta_ili_relative', 0.0)):.3f}, "
        f"γ={float(formula_params.get('gamma_neighbor', 0.0)):.3f}"
    )
    print("-" * 72)
    print(f"{'Group':<8}{'b_g':>12}{'prior_g':>12}")
    print("-" * 32)
    for grp, label in (('child', 'child'), ('adult', 'adult'), ('elderly', 'elderly')):
        b_g, _, _ = _formula_group_keys(formula_params, grp)
    ...
    if pe:
        print("\nMonthly policy parameters policy_m:")
...
def print_and_export_factor_weight_summary(
    ...
    factor_labels = {
        'flu': 'Current influenza conditions',
        'peer': 'Recommendations/peer term',
        'pol': 'Policy outreach',
        'cog': 'Individual cognition',
    ...
    lines: List[str] = []
    lines.append("=" * 80)
    lines.append(f"Factor contribution weights by group: {scenario_label}")
    lines.append("=" * 80)
    lines.append("Note: weights are normalized by each factor's mean absolute logit contribution; individual cognition is 0 when excluded from the current primary formula.")
    ...
    for group in ('child', 'adult', 'elderly'):
        ...
        lines.append("  Questionnaire-factor weights (excluding prior vaccination): " + _share_line(q_share))
        ...
        lines.append("  Mean absolute logit contribution: " + detail)

    text = "\n".join(lines)
    print("\n" + text)

    target_dir = output_dir if output_dir is not None else OUTPUT_CONFIG.get('output_dir', '')
    filename = "factor_contribution_weights_by_group.txt"
    try:
        if target_dir:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write(text)
        print(f"Factor contribution weights by group saved to: {os.path.abspath(report_path)}")
    except OSError as e:
...
def execute_policy_scenario(
    ...
    try:
        ...
        print(f"Warning: unable to create output directory '{output_dir}': {e}")
    ...
    print(f"\nStarting {num_runs} simulation runs ({scenario_label})")
    ...
    print(f"\n{scenario_label}: {num_runs} simulation runs completed")
    ...
    print_survey_weight_summary(scenario_label=scenario_label)
    print_monthly_increments(monthly_increments)
    print_group_prediction_metrics(monthly_increments, scenario_label=scenario_label)
    plot_monthly_vaccination_ci_by_group(monthly_increments, num_runs=num_runs)
    export_monthly_vaccination_ci_csv(monthly_increments, scenario_label=scenario_label)
...
if __name__ == "__main__":
    ...
    policy_scenarios = build_policy_shift_scenarios()
    enable_half_month = OUTPUT_CONFIG.get('enable_policy_half_month_scenarios', True)
    if enable_half_month:
        policy_desc = (
            "The following scenarios will run in sequence: "
            + ", ".join(label for _, label in policy_scenarios)
            + f"; results will be written to {OUTPUT_CONFIG['output_dir']} subdirectories."
        )
    else:
        policy_desc = (
            f"Half-month policy advance/delay scenarios are disabled; running baseline only ({POLICY_SHIFT_SCENARIO_BASE[1]}), "
            f"results will be written to {OUTPUT_CONFIG['output_dir']}/{POLICY_SHIFT_SCENARIO_BASE[1]}."
        )
    print(f"Policy scenarios: {policy_desc}")
    print(
        "ILI mapping: step -> day -> simulation week (week_index_from_step) -> CSV epidemiological week: "
        "simulation week 1 = table week 31, followed by 32...52 -> 1...13 (see csv_epi_week_from_simulation_week / SIMULATION_EPI_WEEK_SEQUENCE)"
    )
    wcsv = ILI_CONFIG.get('weekly_csv') or '(not configured)'
    print(f"  weekly_csv: {wcsv}")
    print("Autonomous infection rate r_auto is calculated from the current week's lambda (CSV); if CSV loading fails or the week is missing, r_auto=0 for that step.")
    print(f"  ILI CSV load failed: {ILI_WEEKLY_CSV_LOAD_FAILED}")
    print("Note: P_n (neighbor transmission) is printed by month below; r_auto is not configured monthly and is derived per step from the ILI CSV at runtime.")
    for month in range(1, SIMULATION_MONTHS + 1):
        params = MONTHLY_PARAMS[month]
        print(f"\nMonth {month} (steps {MONTH_BOUNDARIES[month - 1][0]}-{MONTH_BOUNDARIES[month - 1][1]}):")
    ...
    for policy_shift, scenario_label in policy_scenarios:
        scenario_dir = (
        ...
        execute_policy_scenario(
            nodes_template,
            initial_summary,
            policy_half_month_shift=policy_shift,
            scenario_label=scenario_label,
    ...
    print("\n" + "=" * 80)
    print("All policy-scenario simulations completed. Result directories:")
    for _, scenario_label in policy_scenarios:
        sub = (
            os.path.join(base_output_dir, scenario_label)
            if base_output_dir
            else scenario_label
        )
        print(f"  - {scenario_label}: {os.path.abspath(sub)}")
    print("=" * 80)
